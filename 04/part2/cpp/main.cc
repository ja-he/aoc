...
bool
is_within_range(int num, int lb, int ub);
...
Passport::is_valid(void) const
{
  ...
  // clang-format off
  if (!is_within_range(std::stoi(*this->byr), 1920, 2002)) { return false; }
  if (!is_within_range(std::stoi(*this->iyr), 2010, 2020)) { return false; }
  if (!is_within_range(std::stoi(*this->eyr), 2020, 2030)) { return false; }
  // clang-format on

  const std::string& h          = this->hgt.value();
  int                height_num = std::stoi(h.substr(0, h.length() - 2));
  if (h.substr(h.length() - 2, h.length() - 1) == "cm") {
    if (!is_within_range(height_num, 150, 193))
      return false;
  } else if (h.substr(h.length() - 2, h.length() - 1) == "in") {
    if (!is_within_range(height_num, 59, 76))
      return false;
  } else {
    return false;
  }

  if (!std::regex_match(this->hcl.value(), std::regex("#[0-9a-f]{6}"))) {
    return false;
  }

  if (!std::regex_match(this->ecl.value(),
                        std::regex("amb|blu|brn|gry|grn|hzl|oth"))) {
    return false;
  }

  if (!std::regex_match(this->pid.value(), std::regex("[0-9]{9}"))) {
    return false;
  }
